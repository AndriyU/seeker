...
class Address(db.Model, BaseModel):
    __table_args__ = (
        db.Index('address_resident_uid', 'resident_uid', postgresql_using='hash'),
        db.Index('address_zip_code', 'zip_code', postgresql_using='hash'),
        db.Index('address_latitude', 'latitude', postgresql_using='hash'),
        db.Index('address_longitude', 'longitude', postgresql_using='hash'),
        db.Index('address_id', 'id', postgresql_using='hash'),
    )
    __tablename__ = 'address'
    __public__ = ['id', 'resident_uid', 'zip_long', 'zip_code', 'state', 'city', 'street_address', 'street_address2',
    ...
    resident_uid = db.Column(db.BigInteger)
